...
"""
PARAM ç”¨æ¥å­˜æ”¾æœºå™¨ç‹—çš„å‚æ•°é™åˆ¶èŒƒå›´
PARAM is used to store the parameter limit range of the robot dog
"""

PARAM = {
    "TRANSLATION_LIMIT": [35, 19.5, [75, 115]], # X Y Z å¹³ç§»èŒƒå›´ Scope of translation
    "ATTITUDE_LIMIT": [20, 22, 16],           # Roll Pitch Yaw å§¿æ€èŒƒå›´ Scope of posture
    "LEG_LIMIT": [35, 19.5, [75, 115]],         # è…¿é•¿èŒƒå›´ Scope of the leg
    "MOTOR_LIMIT": [[-73, 57], [-66, 93], [-31, 31]], # ä¸‹ ä¸­ ä¸Š èˆµæœºèŒƒå›´ Lower, middle and upper steering gear range
    "PERIOD_LIMIT": [[1.5, 8]],
    "MARK_TIME_LIMIT": [10, 35],  # åŽŸåœ°è¸æ­¥é«˜åº¦èŒƒå›´ Stationary height range
    "VX_LIMIT": 25,    # Xé€Ÿåº¦èŒƒå›´ X velocity range
    "VY_LIMIT": 18,    # Yé€Ÿåº¦èŒƒå›´ Y velocity range
    "VYAW_LIMIT": 100  # æ—‹è½¬é€Ÿåº¦èŒƒå›´ Rotation speed range
}
...
def conver2u8(data, limit, mode=0):
    """
    å°†å®žé™…å‚æ•°è½¬åŒ–ä¸º0åˆ°255çš„å•å­—èŠ‚æ•°æ®
...
class DOGZILLA():
    """
    åœ¨å®žä¾‹åŒ–DOGZILLAæ—¶éœ€è¦æŒ‡å®šä¸Šä½æœºä¸Žæœºå™¨ç‹—çš„ä¸²å£é€šè®¯æŽ¥å£
    ...
    def translation(self, direction, data):
        """
        ä½¿æœºå™¨ç‹—è¶³ç«¯ä¸åŠ¨ï¼Œèº«ä½“è¿›è¡Œä¸‰è½´å¹³åŠ¨
    ...
    def attitude(self, direction, data):
        """
        ä½¿æœºå™¨ç‹—è¶³ç«¯ä¸åŠ¨ï¼Œèº«ä½“è¿›è¡Œä¸‰è½´è½¬åŠ¨
    ...
    def action(self, action_id):
        """
        ä½¿æœºå™¨ç‹—ç‹—æŒ‡å®šçš„é¢„è®¾åŠ¨ä½œ
    ...
    def reset(self):
        """
        æœºå™¨ç‹—åœæ­¢è¿åŠ¨ï¼Œæ‰€æœ‰å‚æ•°æ¢å¤åˆ°åˆå§‹çŠ¶æ€
    ...
    def leg(self, leg_id, data):
        """
        æŽ§åˆ¶æœºå™¨ç‹—çš„å•è…¿çš„ä¸‰è½´ç§»åŠ¨
    ...
    def perform(self, mode):
        """
        å¼€å¯/å…³é—­æœºå™¨ç‹—å¾ªçŽ¯åšåŠ¨ä½œçŠ¶æ€
    ...
    def motor_speed(self, speed):
        """
        è°ƒèŠ‚èˆµæœºè½¬åŠ¨é€Ÿåº¦ï¼Œåªåœ¨å•ç‹¬æŽ§åˆ¶èˆµæœºçš„æƒ…å†µä¸‹æœ‰æ•ˆ
    ...
    def read_motor(self, out_int=False):
        """
        è¯»å–12ä¸ªèˆµæœºçš„è§’åº¦ Read the angles of the 12 steering gear
        """
    ...
    def __unpack_imu_raw(self):
        '''
        å°†ä¸²å£ç¼“å­˜æ•°æ®è½¬åŒ–æˆIMUåŽŸå§‹æ•°æ®, è¿”å›ž[accX, accY, accZ, gyroX, gyroY, gyroZ, roll, pitch, yaw]
    ...
    def read_imu_raw(self):
        """
        è¯»å–IMUåŽŸå§‹æ•°æ®, è¿”å›žå€¼ï¼š[accX, accY, accZ, gyroX, gyroY, gyroZ, roll, pitch, yaw]
    ...
    def calibration(self, state):
        """
        ç”¨äºŽè½¯ä»¶æ ‡å®šï¼Œè¯·è°¨æ…Žä½¿ç”¨ï¼ï¼ï¼ For software calibration, please use with caution!!!
        """
        if state:
